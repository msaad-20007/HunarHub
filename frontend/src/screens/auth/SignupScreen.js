  ...
  return (
    <View style={styles.pickerWrap}>
      ...
      <TouchableOpacity style={styles.pickerSelector} onPress={() => setVisible(true)} activeOpacity={0.7}>
        <Ionicons name={selected.icon} size={18} color={COLORS.primary} style={{ marginRight: 8 }} />
              ...
              renderItem={({ item }) => (
                ...
                >
                  <View style={styles.optionLeft}>
                    <Ionicons name={item.icon} size={18} color={item.label === selectedValue ? COLORS.primary : COLORS.textSecondary} style={{ marginRight: 10 }} />
                    <Text style={[styles.optionText, item.label === selectedValue && { color: COLORS.primary }]}>{item.label}</Text>
                  </View>
                  {item.label === selectedValue && <Ionicons name="checkmark" size={18} color={COLORS.primary} />}
                </TouchableOpacity>
              )}
            />
          </View>
...
const SignupScreen = ({ navigation }) => {
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    try {
      const userData = { name, email, password, phone, city, address_street: addressStreet, address_zip: addressZip, role, dob: dob.toISOString().split('T')[0] };
      if (role === 'WORKER') {
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <Animated.View style={[styles.content, { opacity: fadeAnim, transform: [{ translateY: slideY }] }]}>
          ...
          <View style={styles.card}>
            ...
            <CustomInput label="Street Address" placeholder="e.g. 12-B, Street 5, G-10/1" value={addressStreet} onChangeText={setAddressStreet} />
            <CustomInput label="ZIP / Postal Code" placeholder="e.g. 44000" value={addressZip} onChangeText={setAddressZip} keyboardType="numeric" />
...
const styles = StyleSheet.create({
  ...
  scroll: { flexGrow: 1, paddingHorizontal: SIZES.padding, paddingTop: SIZES.extraLarge * 4, paddingBottom: SIZES.extraLarge * 2 },
  ...
  roleToggle: {
    ...
    marginBottom: SIZES.large, overflow: 'hidden', width: '100%',
  },
  roleBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: SIZES.small + 2, gap: 6 },
  roleBtnActive: { backgroundColor: 'rgba(0,210,255,0.15)' },
  ...
  card: {
    ...
    borderWidth: 1, borderColor: COLORS.border, marginBottom: SIZES.extraLarge,
  },
  ...
  dateBtn: {
    ...
    borderWidth: 1, borderColor: COLORS.border, padding: SIZES.small,
  },
  dateBtnText: { ...FONTS.body, color: COLORS.text },
  workerDivider: { flexDirection: 'row', alignItems: 'center', marginVertical: SIZES.padding },
  ...
  pickerSelector: {
    ...
    paddingHorizontal: SIZES.small, paddingVertical: SIZES.small + 2,
  },
  pickerValue: { ...FONTS.body, color: COLORS.text, flex: 1 },
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' },
  modal: {
    backgroundColor: COLORS.card, borderRadius: SIZES.radius,
    width: '80%', maxHeight: '60%', paddingVertical: SIZES.base,
    borderWidth: 1, borderColor: COLORS.border,
  },
  modalTitle: {
    ...FONTS.large, color: COLORS.primary, textAlign: 'center',
    paddingVertical: SIZES.small, borderBottomWidth: 1,
    borderBottomColor: COLORS.border, marginBottom: SIZES.base,
  },
  option: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: SIZES.small, paddingHorizontal: SIZES.padding },
