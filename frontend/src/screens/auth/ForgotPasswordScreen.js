...
const ForgotPasswordScreen = ({ navigation }) => {
  ...
  const handleSendOtp = async () => {
    ...
    if (!emailReg.test(email.trim())) { Alert.alert('Invalid', 'Please enter a valid email address.'); return; }
    setLoading(true);
    try {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      Alert.alert('Invalid Code', e.message || 'The code is incorrect or expired.');
      setOtp('');
    } finally {
      setLoading(false);
    }
  ...
  const renderStep = () => {
    if (step === 1) return (
      <>
        <View style={s.iconWrap}>
          <LinearGradient colors={['#FF6B3520', '#FF6B3508']} style={s.iconBg}>
            <Text style={s.icon}>📧</Text>
          </LinearGradient>
        </View>
        <Text style={s.title}>Forgot Password?</Text>
        <Text style={s.subtitle}>Enter your email and we'll send{'\n'}a 6-digit verification code</Text>
    ...
    if (step === 2) return (
      <>
        <View style={s.iconWrap}>
          <LinearGradient colors={['#00D2FF20', '#00D2FF08']} style={s.iconBg}>
            <Text style={s.icon}>🔢</Text>
          </LinearGradient>
        </View>
    ...
    if (step === 3) return (
      <>
        <View style={s.iconWrap}>
          <LinearGradient colors={['#00E67620', '#00E67608']} style={s.iconBg}>
            <Text style={s.icon}>🔐</Text>
          </LinearGradient>
        </View>
        <Text style={s.title}>New Password</Text>
        <Text style={s.subtitle}>Create a strong password{'\n'}for your account</Text>
            ...
            <View style={s.strengthRow}>
              {[1,2,3,4].map(i => (
                <View key={i} style={[s.strengthBar,
                  newPass.length >= i * 3 && { backgroundColor:
                    newPass.length < 6 ? '#FF4C4C' : newPass.length < 9 ? '#FFC107' : '#00E676'
                  }
                ]} />
              ))}
              <Text style={s.strengthLabel}>
                {newPass.length < 6 ? 'Weak' : newPass.length < 9 ? 'Fair' : 'Strong'}
              </Text>
            </View>
          )}
...
const s = StyleSheet.create({
  ...
  stepCheck:     { fontSize: 13, color: '#00E676', fontWeight: '900' },
  ...
  emailHighlight: { color: '#00D2FF', fontWeight: '700' },
  ...
  inputWrap: {
    ...
    borderWidth: 1, borderColor: '#162535',
