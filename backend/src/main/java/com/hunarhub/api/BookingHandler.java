...
public class BookingHandler implements HttpHandler {
            ...
            try (Connection conn = DatabaseConnection.getConnection()) {
                ...
                try {
                    PreparedStatement wstmt = conn.prepareStatement(
                        "SELECT u.email, u.name, u.address_city AS city, cat.category_name AS category FROM users u " +
    ...
    private void handleGetByCustomer(HttpExchange exchange, int userId) throws IOException {
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            String sql =
                ...
                "w.worker_id, wu.name AS worker_name, wu.address_city AS worker_city, cat.category_name AS category, w.rating " +
                "FROM bookings b " +
    ...
    private void handleGetByWorker(HttpExchange exchange, int workerId) throws IOException {
        try (Connection conn = DatabaseConnection.getConnection()) {
            String sql =
                "SELECT b.booking_id, b.type, b.booking_date, b.status, " +
                "c.customer_id, cu.id AS customer_user_id, cu.name AS customer_name, cu.phone AS customer_phone, cu.address_city AS customer_city " +
                "FROM bookings b " +
                ...
                if (rows > 0) {
                    ...
                    try {
                        PreparedStatement nstmt = conn.prepareStatement(
                            "SELECT cu.email, cu.name, wu.name AS worker_name, cat.category_name AS category, b.booking_date " +
                            "FROM bookings b " +
                            "JOIN customers c  ON b.customer_id = c.customer_id " +
                            "JOIN users cu     ON c.user_id     = cu.id " +
                            "JOIN workers w    ON b.worker_id   = w.worker_id " +
