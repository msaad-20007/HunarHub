    ...
    public static class RegisterHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            try {
                ...
                String addressStreet = json.optString("address_street", "");
                String addressZip    = json.optString("address_zip", "");
                String role = json.getString("role"); // "WORKER" or "CUSTOMER"

                // Parse dob string "yyyy-MM-dd" → java.util.Date
                java.util.Date dobDate = null;
                String dobStr = json.optString("dob", "");
                if (!dobStr.isEmpty()) {
                    try { dobDate = new java.text.SimpleDateFormat("yyyy-MM-dd").parse(dobStr); }
                ...
                newUser.setAddressZip(addressZip);
