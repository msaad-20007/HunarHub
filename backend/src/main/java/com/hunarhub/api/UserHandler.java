        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if ("WORKER".equals(role)) {
                PreparedStatement wp = conn.prepareStatement(
                    "SELECT w.worker_id, w.cnic, w.whatsapp, c.category_name AS category, w.approval_status, w.rating " +
                    "FROM workers w JOIN categories c ON w.category_id = c.category_id " +
                    "WHERE w.user_id = ?");
                wp.setInt(1, userId);
